    ...
    int numDecodings(string s) {
        ...
        for (int i = 2; i <= n; i++) {
            if (s[i-1] == '*') {
                ...
                else if (s[i-2] == '*') dp[i] = (dp[i] + dp[i-2] * 15) % MOD;
            } else if (s[i-2] == '*') {
                int digit = s[i-1] - '0';
                if (digit >= 1) dp[i] = (dp[i] + dp[i-1]) % MOD;
                if (digit <= 6) dp[i] = (dp[i] + dp[i-2] * 2) % MOD;
                else            dp[i] = (dp[i] + dp[i-2]) % MOD;
            } else {
                int one = s[i-1] - '0';
                int two = stoi(s.substr(i-2, 2));
                if (1 <= one && one <= 9)   dp[i] = (dp[i] + dp[i-1]) % MOD;
                if (10 <= two && two <= 26) dp[i] = (dp[i] + dp[i-2]) % MOD;
            }
        }
        return dp[n];
    }
