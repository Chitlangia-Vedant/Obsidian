...
Range-based for-loops are a little more interesting, they still work for all kinds of different structures (arrays, lists, trees, maps, etc…). How do these work? They use iterators.
...
We noted that using `operator<` was preferred over `operator!=` when doing numeric comparisons in the loop condition:
