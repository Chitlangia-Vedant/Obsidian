...
One way would be to add a virtual function to Base called getName() (so we could call it with a Base pointer/reference, and have it dynamically resolve to Derived::getName()). But, we would be polluting our Base class with things that really should only be the concern of the Derived class.
...
However, we’ve made quite a dangerous assumption: that b is pointing to a Derived object. If b wasn’t pointing to a Derived object.
