...
The stack itself is a fixed-size chunk of memory addresses. The [[C++/EXTRA#Mailbox analogy for stack|mailboxes]] are memory addresses, and the “items” we’re pushing and popping on the stack are called **stack frames**. A stack frame keeps track of all of the data associated with one function call.
