...
3. We want single parameter constructor

```java
class Student{
	Student(OneParam){ 
// OneParam->Some DS that can allow you to pass multiple values and each having a specific name 
		// Validation
	}
}
```
...
4. We need something like map + compile time check for name & type 
	- Class/Object
### How to implement
...
### Definition
Builder DP allows you to create an object when we have following constraints
1. Too many attribute
2. Validate before creating an object
### Modification
#### Modification 1
...
class Student{
	private Student(){};
	class Bulder{ //Inner Class
		//Builder Imlementation
	}
}
```
...
Implement some special kind of behavior/action
...
When then are multiple ways (algorithms) of doing something, we might often see violation of SRP & OCP because people might take care of multiple ways by writing multiple if-else-if
### How to implement

Rather than implementing behavior in a method, implement that in a separate class.
For every way to do the particular thing, create a class
...
class WalkPathCalculator{
	<<New PathCalculator added>>
}
PathCalculator<|--CarPathCalculator
PathCalculator<|--BikePathCalculator
PathCalculator<|--TrainPathCalculator
PathCalculator<|--WalkPathCalculator
```
```java
class PathCalculatorFactory{
	CarPathCalculator cpc=new CarPathCalculator();
	BikePathCalculator bpc=new BikePathCalculator();
	TrainPathCalculator tpc=new TrainPathCalculator();
	WalkPathCalculator wpc=new WalkPathCalculator();
	//Singleton
	PathCalculator getPCForMode(mode){
		if(mode==car) return cpc;
		else if(mode==bike) return bpc;
		else if(mode==train) return tpc;
		else if(mode==walk) return wpc;
	}
}
class GoogleMaps{
	findPath(src,dest,mode){
		PathCalculator pc=PathCalculatorFactory.getPCForMode(mode);
...
### Problem Statement
1. When something happens, I might want to do a lot of things, every time I need to add/remove something I will have to update my code and compile
2. I can't add/remove things @ runtime
...
A --> C
```
**Currently**-> Class A might want to do multiple things (Facade Classes) when an when happens.

### How to Implement

**Solution** -> Multiple class might want to execute themselves when an imp. event happens
...
class A {
	<<Publisher>>
    impEvent() event has happend here
}
```
...
```
---
1. For all the classes that are subscriber for the event, make them implement the interface
2. Create a method in publisher that allows subscriber to register/deregister
```mermaid
classDiagram
...
class Amazon {
    ...
    registerOPS(OrderPlacedSubscriber o)
    unregisterOPS(OrderPlacedSubscriber o)
    orderPlaced()
}
...
OrderPlacedSubscriber <|.. InvoiceGen
Amazon --> OrderPlacedSubscriber : subscribers
InvoiceGen --> Amazon : registerOPS(this)
```
...
class Amazon{
	List<OrderPlacedSubscriber> ops{};
	static registerOPS(OrderPlacedSubscriber o){
		ops.add(o);
	}
	unregistorOPS(OrderPlacedSubscriber o){
		ops.remove(o);
	}
	orderPlaced(){ //Event
		for(OPS o:ops){
...
class InvoiceGen implements OrderPlacedSubscriber{
	InvoiceGen(){
		Amazon.resgisterOPS(this);
	}
	onOrderPlaced(...){
		...
	}
